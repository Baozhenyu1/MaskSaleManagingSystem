      ...
      <a-form>
        ...
        >
          <a-input
            ...
            placeholder="地址" v-model="mdl.address" id="phar_addr" name="phar_addr"/>
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
...
<script>
  ...
  import { editStoreBasicInfo } from '@/api/manage'
  import { USERNAME } from '@/store/mutation-types'
  import Vue from 'vue'
  import notification from 'ant-design-vue/es/notification'
  import { editStoreDaysData } from '@/api/manage'
  ...
  export default {
    ...
    methods: {
      ...
      handleOk () {
        ...
        let that = this;
        editStoreBasicInfo({'district': this.detail.district,'phar_id':this.mdl.id,'phar_addr':this.mdl.address,
          "phar_con_per":this.mdl.contactPerson,"phar_con_tel":this.mdl.tel}).then(function(data) {
      ...
      showError(index, error){
        let checking_keys = ['进货量','销售量','损耗量','库存'];
      ...
      checkValueError(){
        let required_keys = ['purchased','sale','loss','balance'];
        let item;
      ...
      updateHistory(){
        let postDict = {'district': this.detail.district,'phar_id':this.detail.id};
        postDict['days_data'] = [];
        let index = 0;
        let len = this.data.length;
        this.data.forEach(item=>{
          postDict['days_data'].push({'phar_date':item.report_time,'purchased':item.purchased,
            'issued':item.sale,'tag':index === len - 1?item.tag:1,
            'loss':item.loss,'balance':item.balance
          })
          index ++;
        });
        let that = this;
